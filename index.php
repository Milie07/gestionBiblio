...
if ($recherche || $categorie) {
  ...
  $sql = "SELECT l.*, c.libelle_categorie
            FROM Livre l
            LEFT JOIN Categorie c ON l.id = c.id
            ORDER BY l.titre";
  $stmt = $pdo->query($sql);
...
$sqlCategories = "SELECT * FROM Categorie ORDER BY libelle_categorie";
  ...
  <header>
    <nav>
      <ul>
        ...
        <li><a href="/public/pages/espace.php">Mon Espace</a></li>
        <li><a href="/public/pages/admin.php">Espace Admin</a></li>
      </ul>
    </nav>
  </header>
      ...
      <div class="catalogue">
        <?php foreach ($livres as $livre): ?>
          ...
          <article class="livre-card">
            ...
            <?php if (!empty($livre['libelle_categorie'])): ?>
              <span class="categorie"><?= htmlspecialchars($livre['libelle_categorie']) ?></span>
            <?php endif; ?>

            <div>
              <span class="disponibilite <?= $livre['disponibilite'] === 'Disponible' ? 'disponible' : 'emprunté' ?>">
